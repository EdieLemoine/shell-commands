...
tag() {
  ...
  # fetch and force update tags
  git fetch --tags -p -f
  [[ $? -eq 0 ]] || return # return if git command failed; cwd is likely not a git repository

  log=$(git log --merges --first-parent master --oneline --decorate --stat --pretty="format:%h%<(18)%d %<(20)%cn %<(30)%s")
  ...
  for line in "${merges[@]}"; do
    ...
    prevLine=$(echo "${log}" | grep -A 1 "^$commitHash" | tail -1)
    ...
    # Find branch name regardless if commit is line or pull request
    if echo "${line}" | grep -q 'Merge branch '; then
      branch=$(echo "${line}" | sed -E "s/^.* Merge branch '(.+)'$/\1/")
    elif echo "${line}" | grep -q 'Automatic line '; then
      branch=$(echo "${line}" | sed -E "s/^.* Automatic line from (.+) ->.*$/\1/")
      to=$(echo "${line}" | sed -E "s/^.* Automatic line from .+ -> (.+)$/\1/")
    else
    ...
    elif [[ ${branch} == release || ${branch} == release/* || ${branch} == feature/* || -n ${prevTag} ]]; then
    ...
    if git tag | grep "${tag}"; then
       read -r "deleteTag?Tag $tag already exits. Delete it first? (y) "
      if [[ -z ${deleteTag} || ${deleteTag} =~ ^[Yy]$ ]]; then
